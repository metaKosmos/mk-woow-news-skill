...
# Commit deployado. O numero da versao nao identifica o codigo: varios commits vivem sob o
# mesmo x.y.z, e no rollback a env da revisao fica certa apontando para outro codigo. Fora
# de um clone git sai vazio, e ai o /version apenas omite o campo em vez de mentir.
SKILL_COMMIT="$(git -C "$(dirname "$0")/.." rev-parse --short HEAD 2>/dev/null || true)"
